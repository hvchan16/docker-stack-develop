import os
from peewee import Model, IntegerField, CharField, Field
...
class EnumField(Field):
    ...
    def __init__(self, enum, *args, **kwargs):
        if type(enum) != set:
            raise TypeError(f"{self.__class__.__name__} Argument enum must be of type set not {type(enum)}")
        self.enum = enum
        super().__init__(*args, **kwargs)
    
    def db_value(self, value):
        if value not in self.enum:
            raise ValueError(f"{self.__class__.__name__} Value not member of enum {value} {list(self.enum)}")
        return value
